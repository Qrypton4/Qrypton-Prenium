...
const FAQS = [
 ...
 { id: "weekend-ftmo", q: "FTMO Standard ou Swing : quelle différence pour le week-end ?", a: "Avec un compte FTMO Standard, les positions ouvertes doivent être clôturées avant le week-end — c'est une règle propre à ce type de compte, pas une limitation de Qrypton. Le compte FTMO Swing, lui, permet de conserver des positions ouvertes pendant le week-end. Cette différence dépend uniquement du type de compte que vous choisissez chez FTMO (le 2-Step peut proposer le type Swing, contrairement au 1-Step) — vérifiez donc bien lequel vous avez avant d'utiliser Qrypton dessus." },
  ...
  { q: "Si j'éteins mon PC pendant qu'une position est ouverte, le robot continue-t-il de la gérer (passage au breakeven, etc.) ?", a: "Non. La gestion dynamique d'une position (dont le passage au seuil de rentabilité, ou breakeven) est effectuée par le robot en temps réel, ce qui suppose que MetaTrader 5 soit ouvert et actif. Par exemple, si vous ouvrez votre PC à 15h30, qu'une position s'ouvre, puis que vous l'éteignez à 18h00, le robot ne pourra plus agir sur cette position pendant la nuit — y compris la passer au breakeven — tant que MT5 n'est pas relancé. Seul le stop loss initial, placé au moment de l'ouverture du trade, reste actif de son côté chez votre broker. Pour une gestion continue de vos positions, y compris la nuit, la solution recommandée est le VPS (voir la question précédente sur le fonctionnement mobile/tablette)." },
];
